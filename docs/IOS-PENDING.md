...
## 46. 2026-09-18 顶部通透度与流式渲染优化（Build 114）
- 顶部：去掉额外底色遮罩，调低原生模糊效果强度，让字形轮廓更明显；无内容经过时保持页面底色。使用独立明暗主题截图检查，避免只看没有文本经过的页面。
- 流式诊断：真实Mac短回复6段增量间隔21/25/29/29/37ms，说明该次源端持续发送；App已有50ms事件批处理，问题不能简单归因为逐token发布。发现会话派生数组重复排序分组、已显示消息Markdown重复解析，进行缓存优化。
- 优化：每个会话缓存排序分组结果；每条已显示消息保留Markdown解析结果，仅重解析文本实际变化的消息。保留50ms事件批处理、原始内容顺序及最终替换语义。历史重放/超时合并/隐藏/重置均使对应缓存失效。
- 验证：121项iOS测试通过，明暗主题文字覆盖截图已检查，签名归档及导出成功。2026-09-18 10:15（上海时间）TestFlight 1.0(114)上传成功，Apple已接收，等待处理；Delivery UUID：e13f7a0e-0d1f-4f5d-abe0-b1ee6c07de9f。
- 产物：artifacts/ios/blur-stream-build114-20260918/；IPA SHA256：4af81632701ec843f1c43d00fe9ff9f0970692b4883bf2661709737acb244c22。


## 47. 2026-09-18 自适应流式预留空间与75%顶部底色（Build 115）
- 顶部：首页和会话页移除磨砂模糊，统一页面底色75%不透明度；保留原有内容经过时的分隔线规则。
- 流式：按实际布局与增长情况预留可消耗空间，结束后移除剩余留白，避免固定留几行或每次换行直接推高页面。
...
## 49. 2026-09-20 启动/重连时自动恢复读取（Build 116）
...
## 50. 2026-09-21 启动连接状态与顶部透明度（Build 117）
- 启动：已配对 App 在读取本机待处理事务记录、尚未开始 Relay 连接的短暂阶段显示“正在连接”，不再提前显示 Mac 无法访问；收到明确离线或连接失败后仍显示原有排查入口。
- 顶部：首页和会话页纯底色遮罩由 75% 调至 82% 不透明度，仍不使用磨砂；设置页和弹窗的透明度保持不变。
- 验证：Release 配置两项针对性测试通过，覆盖配对启动准备状态和顶部明暗截图。1.0（117）已从隔离源码完成签名归档和导出，IPA SHA-256 `dadcc58debcd87dbb7e57ce5aa3323f9fb2bed8ff93dcf4da4e3458b93fff5d3`；2026-09-21 09:05（上海）Apple 返回 `UPLOAD SUCCEEDED with no errors`，Delivery UUID `c4b49843-3aef-492e-aa72-18502922e21a`。源码已同步 GitHub；现有 Localizable/Relay 未提交修改未进入安装包。
